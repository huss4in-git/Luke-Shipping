export default function Banner() {
    return (
      ...
      >
        ...
        {/* Right image - full height on mobile, takes right portion */}
        ...
        <div className="relative z-10 flex flex-col items-start sm:items-center text-left sm:text-center px-6 sm:px-12 lg:px-6 w-full sm:max-w-2xl sm:mx-auto pr-[45%] sm:pr-12 lg:pr-6">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center sm:justify-center gap-3 w-full">
            {[
              ...
              <button
                key={i}
                className="flex items-center justify-between sm:justify-center gap-2 pl-4 pr-4 py-1.5 sm:px-5 sm:py-2.5 rounded-full border border-white/40 bg-white/10 hover:bg-white/20 text-white text-sm transition-colors duration-150"
                style={{ fontFamily: "nb-thin" }}
              >
                <span>{btn.label}</span>
